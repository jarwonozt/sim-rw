...
use App\Http\Resources\WhatsappBroadcastResource;
...
class WhatsappBroadcastController extends Controller
{
    public function index(): JsonResponse
    {
        ...
        return response()->json(WhatsappBroadcastResource::collection($broadcasts)->response()->getData(true));
    }

    public function store(StoreWhatsappBroadcastRequest $request, FonnteClient $fonnte): JsonResponse
    {
        ...
        if ($phones->isEmpty()) {
            return response()->json([
                'message' => 'Tidak ada penduduk dengan nomor HP pada target yang dipilih.',
            ], 422);
        }
        ...
        return response()->json(['data' => new WhatsappBroadcastResource($broadcast)], 201);
    }
}
