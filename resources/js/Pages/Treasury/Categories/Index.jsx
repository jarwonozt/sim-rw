...
function CategoryFormModal({ show, onClose, category }) {
    ...
    const submit = (e) => {
        ...
        if (isEditing) {
            put(route('treasury-categories.update', category.id), options);
        } else {
            post(route('treasury-categories.store'), options);
        }
    };
...
export default function Index({ categories }) {
    ...
    const destroy = () => {
        router.delete(route('treasury-categories.destroy', confirmDelete.id), {
                        ...
                        {categories.map((category) => (
                            <tr key={category.id}>
                                ...
                                <td className="px-4 py-3">
                                    ...
                                    >
                                        {category.type === 'in' ? 'Kas Masuk' : 'Kas Keluar'}
                                    </span>
                                </td>
                                <td className="px-4 py-3 text-gray-600">
                                    {category.treasuries_count} transaksi
                                </td>
